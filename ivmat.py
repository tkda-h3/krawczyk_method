...
FLOAT_MAX = sys.float_info.max * (10 ** -5)
FLOAT_MIN = sys.float_info.min * (10 ** 5)
...
class ivmat(list):
    ...
    class NotDiagMatchError(Exception):
        pass
    ...
    #     shape: 2D tuple
    #     """
    #     mat = ivmat([[value for _ in range(shape[1])] for _ in range(shape[0])])
    #     return mat
    ...
    def __lt__(self, other): # self.sup < other.inf
        if isinstance(other, int) or isinstance(other, float):
            other = ivmat.uniform_mat(other, self.shape)
        for (x_row, y_row) in zip(self.sup, other.inf):
            for (x, y) in zip(x_row, y_row):
                if not x < y:
                    return False
        return True
    
    def __gt__(self, other): # self.inf > other.sup
        if isinstance(other, int) or isinstance(other, float):
            other = ivmat.uniform_mat(other, self.shape)
        for (x_row, y_row) in zip(self.inf, other.sup):
            for (x, y) in zip(x_row, y_row):
                if not x > y:
                    return False
        return True
    ...
    @property
    def inf(self):# 区間の小さい方
        mat = ivmat([[None for col in range(self.shape[1])]
                     for row in range(self.shape[0])])
        for i in range(mat.shape[0]):
            for j in range(mat.shape[1]):
                mat[i][j] = interval(self[i][j])[0][0]
        return mat

    @property
    def sup(self):# 区間の小さい方
        mat = ivmat([[None for col in range(self.shape[1])]
                     for row in range(self.shape[0])])
        for i in range(mat.shape[0]):
            for j in range(mat.shape[1]):
                mat[i][j] = interval(self[i][j])[0][1]
        return mat
    ...
    def midpoint(self):
        def _validate(iv):
            """
            midpointでスカラーを出力するためにinf, -infをfloatのmax, min定数に置き換える
            """
            low, high = iv[0]
            values = []
            for value in (low, high):
                if value == inf:
                    values.append(FLOAT_MAX)
                elif value == -inf:
                    values.append(FLOAT_MIN)
                else:
                    values.append(value)
        ...
        for i in range(mat.shape[0]):
            for j in range(mat.shape[1]):
                tmp = interval(self[i][j])
                mat[i][j] = _validate(tmp).midpoint
        return mat

    @property
    def diag(self): # 対角成分 shape=(self.shape[0], 1)
        if self.shape[0] != self.shape[1]:
            raise NotDiagMatchError()
        mat = []
    ...
    def get_pinv(self):
        np_pinv = np.around(np.linalg.pinv(np.array(self)), decimals=5)
...
class Krawczyk():
    ...
    def get_R_and_KX(self, X):
        ...
        digit = int(np.log10(ivmat.max(mF1_X.to_interval().abs()))) + 2
        scale = 10.0 ** digit
...
#        mf_grad_x = self.f_grad.apply_args(X).midpoint.to_scalar()  # m(f'(x))
        ...
        logger.info((
            ...
            'R.norm: {}\n'
            'is_empty(KX & X):{}\n'
            'is_in(KX, X):{}\n').format(X, KX, R.norm, ivmat.is_empty(KX & X), ivmat.is_in(KX, X)))
    ...
    def find_all_solution(self, trace=False, cnt_max=1000, max_width=1e-8):
        ...
        return list(map(lambda x: self.refine(x), T)), S_sizes, T_sizes, U_sizes, animation_box
